...
var handleSMS = function(req, res) {
  ...
  User.findOne({ phone: phone }, function(err, user) {
    ...
    if (bodyLc === 'yes' || bodyLc === 'no') {
      voteHandler = _.partialRight(handlePendingVote, user, bodyLc, function(err, user, choice, vote) {
        var msg = util.format("Your vote of '%s' for '%s' has been recorded",
          vote, choice.name);
        var logMsg = util.format("Vote of '%s' for choice '%s' by user '%s' has been recorded",
          vote, choice.name, user.name);
